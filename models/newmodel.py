...
class emb_model_self_attention(tf.keras.Model):
    def __init__(self):
        super().__init__(self)
        self.emb1 = Dense(activation='selu', units=10)
        self.emb2 = Dense(activation='selu', units=3)
        ...
        self.emb4 = Dense(activation='selu', units=3)
        self.num_emb = Dense(activation='selu', units=12)
        self.att0 = Dense(activation='selu', units=14)
        self.att1 = Dense(activation='selu', units=14)
        self.att2 = Dense(activation='softmax', units=5)
        self.linear1 = Dense(activation='selu', units=128, kernel_initializer="random_normal")
        self.dropout = Dropout(0.3)
        self.linear2 = Dense(activation='selu', units=32)
        self.linear3 = Dense(activation='selu', units=16)
    ...
    def call(self, inputs):
        ...
        x = self.att1(x)
        x = self.att2(x)
        x = tf.concat([num_emb*x[:, -5:-4], emb4*x[:, -4:-3], emb3*x[:, -3:-2], emb2*x[:, -2:-1], emb1*x[:, -1:]], 1)
        x = self.linear1(x)
        x = self.dropout(x)
        x = self.linear2(x)
        x = self.linear3(x)
...
class simple_widedeep_model(tf.keras.Model):
    def __init__(self):
        super().__init__(self)
        self.emb1 = Dense(activation='selu', units=10)
        self.emb2 = Dense(activation='selu', units=3)
        ...
        self.emb4 = Dense(activation='selu', units=3)
        self.num_emb = Dense(activation='selu', units=12)
        self.memory = Dense(activation='selu', units=10)
        self.linear1 = Dense(activation='selu', units=128, kernel_initializer="random_normal")
        self.dropout = Dropout(0.3)
        self.linear2 = Dense(activation='selu', units=32)
        self.linear3 = Dense(activation='selu', units=16)
...
def get_emb_model_self_attention(x_train=None, y_train=None, x_test=None, y_test=None, is_train=False):
    model = emb_model_self_attention()
    print(model)
    ...
    if is_train:
        ...
        print(result)
        draw_history(history)
        model.save_weights("./models/emb_self_attention_model")
        print(model.summary())
...
def get_widedeep_model(x_train=None, y_train=None, x_test=None, y_test=None, is_train=False):
    model = simple_widedeep_model()
    print(model)
    callback = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=35)
    lr_schedule = keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=2e-3,decay_steps=10000,decay_rate=0.95)
    opt = tf.keras.optimizers.Adam(learning_rate=lr_schedule, decay=0.)
    ...
    if is_train:
        ...
        print(result)
        draw_history(history)
        model.save_weights("./models/widedeep_model")
        print(model.summary())
